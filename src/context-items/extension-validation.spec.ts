...
import { ExtensionValidation } from './extension-validation';

let context: Context;
let validator: jest.Mock;
let validation: ExtensionValidation;
const meta: Meta = {
  req: { cookies: { foo: 'bar' } },
  location: 'cookies',
  path: 'foo',
...
beforeEach(() => {
  ...
  validator = jest.fn();
  validation = new ExtensionValidation(validator, { customOption: true }, false);
...
const createSyncTest = (options: { returnValue: any; addsError: boolean }) => async () => {
  ...
  expect(validator).toBeCalledWith('bar', { customOption: true }, meta);
};
...
describe('when negated', () => {
  beforeEach(() => {
    validation = new ExtensionValidation(validator, { customOption: true }, true);
    validation.message = 'nope';
  });
